...
// DFS 
class Solution {
    public void DFS(int u, List<List<String>> accounts, boolean[] vis, Set<String> set, int n){
        
        vis[u] = true;

        for(int i = 1; i < accounts.get(u).size(); i++){
            String s = accounts.get(u).get(i);
            set.add(s);
        }
        ...
        for(int v = 0; v < n; v++){
            ...
            for(int i = 1; i < accounts.get(v).size(); i++){
                String str = accounts.get(v).get(i);

                if(set.contains(str)){
                    DFS(v, accounts, vis, set, n);
                }
            }
        }
    ...
    public List<List<String>> accountsMerge(List<List<String>> accounts) {
        ...
        for(int u = 0; u < n; u++){
            ...
            if(!vis[u]){
                ...
                String name = accounts.get(u).get(0);
                DFS(u, accounts, vis, set, n);
